...
class Metric:
    ...
    def all_metrics(self):
        self.metrics = ["mae_global", "mae_boundary", "mae_component",
         ...
         "global_image_spearmanr", "mape_global"]
        return self.metrics
    ...
    def data_info(self):
        ...
        self.comp_pixel_size = np.round(self.comp_size / self.L * 64).astype(int)
            ...
            for i in range(comp_num):
                ...
                if mesh == 'uniform':
                    ...
                    if xs is None or ys is None:
                        xs = torch.linspace(0, domain_length, steps=64)  # 生成200个均匀排列的数
                        ys = torch.linspace(0, domain_length, steps=64)
